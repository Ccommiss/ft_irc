...
void    send_channel(Server &s, User *u, std::vector<std::string> arg)
{
    std::string arg_1 = *(arg.begin() + 1);
    {
        out ("Sending to channel >>> " << arg_1)
        std::string txt = set_prefix(u, arg);
        for (std::map<std::string *, User *>::iterator it = s.chans[arg_1]->get_users().begin(); it != s.chans[arg_1]->get_users().end(); it++)
        {
            out ("HEHE")
            out ("USERS" << it->first);
            if (it->second != u)
